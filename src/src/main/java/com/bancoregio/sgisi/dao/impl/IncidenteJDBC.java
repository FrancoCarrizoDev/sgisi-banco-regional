...
public class IncidenteJDBC implements IncidenteDAO {
    ...
    public int insertar(Incidente i, Connection c) throws SQLException {
        ...
        try(var ps=c.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)){
            ps.setInt(1,i.getTipo().id()); ps.setInt(2,i.getSeveridad().id()); ps.setInt(3, estadoIdPorNombre(i.getEstado().getNombre(), c)); ps.setInt(4,i.getActivo().id());
            ps.setString(5,i.getDescripcion()); ps.setTimestamp(6,Timestamp.valueOf(i.getFechaDeteccion())); ps.setTimestamp(7,Timestamp.valueOf(i.getFechaVencimientoSla())); ps.executeUpdate();
            try(var gk=ps.getGeneratedKeys()){ gk.next(); return gk.getInt(1); }
        }
    }
    /** {@inheritDoc} */
    public void actualizarEstado(int incidenteId, int estadoId, LocalDateTime fechaCierre, Connection c) throws SQLException {
        try(var ps=c.prepareStatement("UPDATE incidente SET estado_id=?, fecha_cierre=? WHERE id=?")){ ps.setInt(1,estadoId); if(fechaCierre==null) ps.setNull(2, Types.TIMESTAMP); else ps.setTimestamp(2,Timestamp.valueOf(fechaCierre)); ps.setInt(3,incidenteId); ps.executeUpdate(); }
    }
    /** {@inheritDoc} */
    public Optional<Incidente> buscarPorId(int id) throws SQLException { List<Incidente> list=listarInterno(" WHERE i.id=? ", ps->ps.setInt(1,id)); return list.isEmpty()?Optional.empty():Optional.of(list.get(0)); }
    /** {@inheritDoc} */
    public List<Incidente> listar(Integer estadoId, Integer severidadId) throws SQLException {
        ...
        return listarInterno(where, ps->{ int idx=1; if(estadoId!=null) ps.setInt(idx++,estadoId); if(severidadId!=null) ps.setInt(idx,severidadId); });
    }
